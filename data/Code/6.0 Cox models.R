...
toc() # time: 1047.444 sec elapsed,  17 min
...
f1 <- plots$birth_preterm + labs(title = "A. Preterm (<37 weeks) last week")
...
# Remove previuos objects with large size
rm(bw_data_lw, model_fit, plots_save, plots, p, 
    results_cox, results_list, 
    table_models, results_filtered, data_subset, combinations, formula)
...
length(unique(bw_data_lm$id))

bw_data_lm$vulnerability <- droplevels(
  bw_data_lm$vulnerability[bw_data_lm$vulnerability != "Alta"]
)
...
combinations <- expand.grid(dependent_vars[1], heatwave_vars[1:2], stringsAsFactors = FALSE)

tic()
results_list <- future_lapply(seq_len(nrow(combinations)), function(i) {
  dep_var <- combinations[i, 1]
  hw_var <- combinations[i, 2]
  fit_cox_model(dep_var, hw_var, data=data)
}, data = bw_data_lm)
toc() # time: 1047.444 sec elapsed,  17 min
...
ggsave(plots_save,
  filename = paste0("Output/", "Models/", "PTB_COX_SUP_LM", ".png"), 
  res = 300,
  width = 30,
  height = 25,
  units = 'cm',
...
ggsave(plots$birth_preterm + labs(title = "A. Preterm (<37 weeks)"),
       filename = paste0("Output/", "Models/", "PTB_COX_lm", ".png"), # "Preterm_trendsrm1991"
       res = 300,
       width = 20,
       height = 15,
       units = 'cm',
...
ggsave(plots$birth_very_preterm + labs(title = "B. Very Preterm (28-32 weeks)"),
  filename = paste0("Output/", "Models/", "PTB_very_COX_lm", ".png"), # "Preterm_trendsrm1991"
  res = 300,
  width = 20,
  height = 15,
  units = 'cm',
...
ggsave(plots$birth_moderately_preterm + labs(title = "C. Moderately Preterm (32-33 weeks)"),
  filename = paste0("Output/", "Models/", "PTB_moderate_COX_lm", ".png"), # "Preterm_trendsrm1991"
  res = 300,
  width = 20,
  height = 15,
  units = 'cm',
...
ggsave(plots$birth_late_preterm + labs(title = "D. Late Preterm (34-37 weeks)"),
       filename = paste0("Output/", "Models/", "PTB_late_COX_lm", ".png"), # "Preterm_trendsrm1991"
       res = 300,
       width = 15,
       height = 15,
       units = 'cm',
